...
$(document).ready(function() {
	$('.enemies a').click(function() {
		$('#stats_months').load(
			'ajax/get_ladder_statistics_months.php',
			{ mode: 'enemies', player: $(this).text() },
	...
	$('#stats_months a').live('click', function() {
		$('#stats_days').load(
			...
			{ mode: 'enemies', player: $(this).attr('p'), year: $(this).attr('y'), month: $(this).attr('m') },
	...
	$('#stats_days a').live('click', function() {
		$('#stats_games').load(
			...
			{ mode: 'enemies', player: $(this).attr('p'), year: $(this).attr('y'), month: $(this).attr('m'), day: $(this).attr('d') }
		);
	});
});
</script>
<?php
	...
	ArghPanel::begin_tag('Ladder - Adversaires');
	
	$orders = array("SUM(A.xp) DESC", "SUM(A.xp) ASC");
	
	foreach ($orders AS $order) {
	
		$allies = LadderStatisticsModule::get_enemies(ArghSession::get_username(), $order, 0, 10);
	
		$count = 0;
		echo '<br /><table class="listing enemies">';
