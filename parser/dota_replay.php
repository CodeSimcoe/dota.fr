<?php
	...
	class DotaPlayer {
		...
		var $name, $color, $hero;
		var $kills, $deaths, $creepskills, $creepsdenies;
	...
	class DotaParser {
		...
		var $replayPath, $replayFile, $replay, $definition;
					...
					switch ($action) {
							...
							if (isset($this->definition->heroes[$item])) {
								if (isset($this->game["modes"]["cm"]) OR isset($this->game["modes"]["xl"])) {
									if ($this->players[$pid]["team"] == 0 AND $this->players[$pid]["color"] == "blue" AND !isset($this->bsen[$item])) {
										...
										$this->bsen[$item]["time"] = convert_time($this->time);
										...
										$this->bsco[$item]["time"] = convert_time($this->time);
									}
								} else {
						...
						case 0x6B:
							...
							if ($this->players[$dotastats['player_id']]) {
								switch($dotastats['statid']) {
									case 1:
										$this->players[$dotastats['player_id']]['kills'] = $dotastats['statval'];
										break;
									case 2:
										$this->players[$dotastats['player_id']]['deaths'] = $dotastats['statval'];
										break;
									case 3:
										$this->players[$dotastats['player_id']]['creepskills'] = $dotastats['statval'];
										break;
									case 4:
										$this->players[$dotastats['player_id']]['creepsdenies'] = $dotastats['statval'];
										break;
								}
							}
							$alen += 4;
