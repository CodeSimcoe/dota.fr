	...
	class requestVars{
	   ...
	   function getVarAlphaNum($param, $max, $default = NULL){
		   if (isset($this->source[$param])) { // check if parameter is set
			   ...
			   preg_match("/^[A-Za-z0-9]*$/",$this->source[$param],$arr);
	   ...
	   //Usernames
	   function getVarUsername($param, $max = 25, $default = NULL) {
		if (isset($this->source[$param])) {
			preg_match('`^[a-zA-Z0-9_\[\]\-\.]+$`', $this->source[$param], $arr);
			if (!empty($arr)) return substr($arr[0], 0, $max);
		}
		return $default;
	   }
	   ...
	   //Password
	   function getVarPassword($param) {
		return htmlspecialchars($this->source[$param]);
	   }
